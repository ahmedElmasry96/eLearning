...
@section('page-header')
	...
	<div class="breadcrumb-header justify-content-between">
		<div class="my-auto">
			<div class="d-flex">
				<h4 class="content-title mb-0 my-auto">Pages</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ @lang('dashboard/admins.edit', ['name' => $admin->name])</span>
			</div>
		</div>
							...
							<form method="post" action="{{route('admins.update', $admin->id)}}" enctype="multipart/form-data">
								...
								<div class="form-group">
									<div class="row row-sm">
										<div class="col-sm-6">
											<label class="main-content-label tx-11 tx-medium tx-gray-600">@lang('dashboard/app.name')</label> <input class="form-control" required type="text" name="name" value="{{$admin->name}}">
											@if($errors->has('name'))
												<p class="text-danger">{{ $errors->first('name') }}</p>
											@endif
										</div>
										<div class="col-sm-6">
											<label class="main-content-label tx-11 tx-medium tx-gray-600">@lang('dashboard/app.email')</label> <input class="form-control" required type="email" name="email" value="{{$admin->email}}">
											@if($errors->has('email'))
								...
								<div class="form-group">
									<div class="row row-sm">
										<div class="col-sm-6">
											...
											@endif
											@if($admin->image)
											<img class="mt-3" src="{{url($admin->image)}}" alt="admin image" width="150" height="150">
											@endif
										</div>
										<div class="col-sm-6">
											...
											<select class="form-control" name="role">
												...
												@foreach($roles as $role)
													<option {{count($admin->roles->pluck('name')) > 0 && $admin->roles->pluck('name')[0] == $role->name ? 'selected' : ""}} value="{{$role->name}}" >{{$role->name}}</option>
												@endforeach
											</select>
											@if($errors->has('role'))
