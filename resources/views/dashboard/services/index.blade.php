...
@section('page-header')
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<div class="d-flex">
							<h4 class="content-title mb-0 my-auto">Pages</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ @lang('dashboard/sidebar.services')</span>
						</div>
					</div>
				...
				<div class="card">
					...
					<div class="card-header pb-0">
						<div class="d-flex justify-content-between">
							<h4 class="card-title mg-b-0">@lang('dashboard/sidebar.services')</h4>
							@if(auth()->user()->can('create service'))
								<a href="{{route('services.create')}}">
									<button class="btn btn-info btn-with-icon"><i class="typcn typcn-plus"></i>@lang('dashboard/services.add')</button>
								</a>
							@endif
						</div>
					</div>
							...
							<table class="table text-md-nowrap" id="example1">
								...
								<tbody>
									@foreach($services as $index => $service)
										<tr>
											<td>{{$index + 1}}</td>
											<td><img src="{{url($service->image)}}" width="50" height="50" alt="service image"></td>
											<td>{{$service->title}}</td>
